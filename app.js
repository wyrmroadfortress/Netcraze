...
function render(){
  ...
  document.querySelectorAll('.product-card.clickable').forEach(card=>{const go=()=>location.href=`product-v2.html?sku=${encodeURIComponent(card.dataset.sku)}`;card.addEventListener('click',go);card.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();go()}})});
