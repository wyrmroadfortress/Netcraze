...
async function init(){
  ...
  $('#productView').innerHTML=`<div class="product-detail-head"><div><div class="eyebrow">${esc(s?.type||'Оборудование Netcraze')}</div><h1 class="detail-title">${esc(p.name)}</h1><div class="detail-sku">Артикул: ${esc(p.sku||'—')}</div><p class="detail-description">${esc(s?.description||'Характеристики модели будут добавлены позже.')}</p><a class="official-btn" href="${esc(url)}" target="_blank" rel="noopener noreferrer">Открыть на официальном сайте <span>↗</span></a></div><div class="detail-price">${money(p.price,p.currency)}</div></div><div class="detail-grid"><section><h2>Характеристики</h2><div class="spec-table">${specRows||'<div class="spec-row"><div class="spec-key">Данные</div><div class="spec-value">Уточняются</div></div>'}</div></section><aside><div class="stock-card"><div class="stock-card-title">Наличие</div>${rows||'<div class="stock-row"><div class="city">Нет доступных остатков</div></div>'}<div class="stock-total"><div class="label">Всего доступно</div><div class="num">${total(p)} <small>шт.</small></div></div></div></aside></div>`;
}
